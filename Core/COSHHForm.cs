...
namespace COSHH_Generator.Core
{
    class COSHHForm
    {
        ...
        public static async Task<string> Generate(string title, string name, string college, string year, string date,
            ...
            List<SubstanceEntry> substances, string outputPath)
        {
            try
            {
                ...
                using (WordprocessingDocument doc = WordprocessingDocument.Open(outputPath, true))
                {
                    ...
                    foreach (var substanceEntry in substances)
                    {
                        ...
                        SafetyData? sds = null;
                        if (substanceEntry.safetyData is not null)
                        {
                            sds = substanceEntry.safetyData;
                        }
                        else if (substanceEntry.extractionTask is not null)
                        {
                            sds = await substanceEntry.extractionTask;
                        }
                        else
                        {
                            sds = new SafetyData();
                        }
                        substanceEntry.Amount = substanceEntry.Amount.Trim();
                        var amount = string.IsNullOrEmpty(substanceEntry.Amount) ? "N/A" : substanceEntry.Amount;
