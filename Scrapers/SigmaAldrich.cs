...
namespace COSHH_Generator.Scrapers
{
    ...
    static class SigmaAldrich
    {
        ...
        {
            using (var request = new HttpRequestMessage(HttpMethod.Get, "https://www.sigmaaldrich.com/"))
            {
                try 
                {
                    // Send a HEAD request to the server to initialize and cache the necessary resources.
                    HttpResponseMessage response = await Client.SendAsync(request);
                    response.EnsureSuccessStatusCode(); // Optionally check if the request was successful.
                }
                catch (Exception ex)
                {
                    // Handle exceptions or log errors as necessary.
                    Trace.WriteLine($"Error warming up HttpClient: {ex.Message}");
                }
            }
        }

        static public async Task<List<Result>> SearchAsync(string query, CancellationToken cancelToken)
        {
